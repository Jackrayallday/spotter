...
import type { TrainingPlan, UserProfile } from "./types.js";
  ...
  try {
    ...
    for (const [index, model] of PLAN_MODELS.entries()) {
      ...
      try {
        ...
        logCompletionShape(completion, attempt);

        const firstChoice = completion.choices?.[0];
        if (!firstChoice) {
          console.warn(
            `[AI] Malformed response (attempt ${attempt}): expected completion.choices[0], but no choices were returned.`,
          );
          console.warn(
            `[AI] Malformed response preview (attempt ${attempt}):`,
            stringifyForLogs(completion),
          );
          lastError = new Error("AI response did not include choices");
          continue;
        }
...
function logCompletionShape(completion: unknown, attempt: number) {
  const response = completion as Record<string, any>;
  const choices = response?.choices;

  console.log(
    `[AI] Completion shape (attempt ${attempt}):`,
    JSON.stringify(
      {
...
function logAiError(error: unknown, attempt: number) {
  if (!(error instanceof Error)) {
    console.warn(`[AI] Non-error thrown (attempt ${attempt}):`, stringifyForLogs(error));
    return;
  }
  ...
  console.warn(
    `[AI] Error diagnostics (attempt ${attempt}):`,
    JSON.stringify(
      {
        name: details.name,
...
function formatPlanResponse(
  ...
  return {
    ...
    progression:
      ...
      "This went to the fallback text. Your plan is not unique. Contact customer support",
  };
}
